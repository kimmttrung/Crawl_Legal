...
> **An toàn:** collection đích là `law_2026`, **không bao giờ** đụng tới collection cũ `vietnamese_laws` (có `assert` chặn cứng). Point id = `uuid5(unique_article_id)` ⇒ chạy lại notebook **không nhân đôi** dữ liệu (idempotent upsert). Đặt `RECREATE = True` để xoá-tạo lại collection từ đầu (mặc định `False`).
...
```jsonc
{
  "id": "49/2005/QH11_Điều 1",
  "text": "Phạm vi điều chỉnh\nLuật này điều chỉnh...",
  "metadata": {
    "unique_article_id": "49/2005/QH11_Điều 1",
    "article_id": "Điều 1",
    "doc_number": "49/2005/QH11",
    "title": "Luật Các công cụ chuyển nhượng 2005",
    "context": { "part": null, "chapter": "Chương I NHỮNG QUY ĐỊNH CHUNG", "section": null },
    "clause_num": null
  }
}
```
...
3. Vào **Settings → Add-ons → Secrets**, thêm 2 secret: `QDRANT_URL` và `QDRANT_API_KEY`.
